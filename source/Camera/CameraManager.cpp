 ...
 */
CameraManager::ActionInstance::~ActionInstance() {
    interpolant = nullptr;
    action = nullptr;
    target = nullptr;
}
 ...
 */
bool CameraManager::isActive(std::string key) const {
 ...
 */
bool CameraManager::activate(std::string key,
 ...
 */
bool CameraManager::remove(std::string key) {
    auto action = _actions.find(key);
...
void CameraManager::update(float dt) {
    ...
    for(auto it = _actions.begin(); it != _actions.end(); ++it) {
        ...
        CameraAction* action = instance->action.get();
        float current = 1.0;
        float future  = 1.0;
 ...
 */
bool CameraManager::isPaused(std::string key) {
    auto action = _actions.find(key);
    if (action == _actions.end()) {
        return false;
    }
 ...
 * @param key       The identifying key
 */
void CameraManager::pause(std::string key) {
    auto action = _actions.find(key);
    if (action == _actions.end()) {
        return;
    }
 ...
 * @param key       The identifying key
 */
void CameraManager::unpause(std::string key) {
    auto action = _actions.find(key);
    if (action == _actions.end()) {
        return;
    }
 ...
 */
void CameraManager::clearAllActions(const std::shared_ptr<Camera>& target) {
 ...
 */
void CameraManager::pauseAllActions(const std::shared_ptr<Camera>& target) {
    auto set = _keys.find(target.get());
    if (set == _keys.end()) {
        return;
    }
 ...
 * @param target    The node to pause animating
 */
void CameraManager::unpauseAllActions(const std::shared_ptr<Camera>& target) {
    auto set = _keys.find(target.get());
    if (set == _keys.end()) {
        return;
    }
 ...
 */
std::vector<std::string> CameraManager::getAllActions(const std::shared_ptr<Camera>& target) const {
